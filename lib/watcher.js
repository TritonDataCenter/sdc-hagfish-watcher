...
Watcher.prototype.resetState = function () {
    this.vms = {};
    this.network = {};
    this.disk = {};
    this.vmPathsToZone = {};
...
Watcher.prototype.emitVirtualMachineState = function () {
    var self = this;
    ...
    async.waterfall([
        ...
        function (vms, cb) {
            var vm_count = Object.keys(vms).length;
            for (var v in vms) {
                self.emit('vm-update', vms[v]);
            }
            self.emit('vm-summary', self.makeSummary(vm_count));
            self.log.info('Emitted %d vm updates.', vm_count);
            cb();
        }
    ],
    function (err) {
...
Watcher.prototype.makeSummary = function (vm_count) {
    var self = this;

    var summary = JSON.parse(JSON.stringify(self.include));

    summary.timestamp = (new Date()).toISOString();
    summary.vm_count = vm_count;
    summary.type = 'summary';
    summary.v = self.data_version;
